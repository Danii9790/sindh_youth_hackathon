...
const QUOTA_LIMITS = {
  REQUESTS_PER_MINUTE: parseInt(process.env.OPENAI_REQUESTS_PER_MINUTE || '15'),
...
function checkRateLimit(): { allowed: boolean; waitTime?: number; fallbackMessage?: string } {
  ...
  if (process.env.OPENAI_RATE_LIMIT_ENABLED !== 'true') {
    return { allowed: true };
  }
...
/* ============================================================
   1️⃣ TEXT-BASED SYMPTOM ANALYSIS (GPT-4o-mini)
   ============================================================ */
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are **Dr. AI**, a senior physician with 25 years of clinical experience.
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an **Expert Medical Imaging Specialist** with deep experience in X-rays, scans, lab reports, and medical images.`
        },
        {
          role: "user",
          content: [
            {
              type: "text",
              text: `Analyze the uploaded image/report and follow this exact structure:
...
export const chatWithMediAI = async (
  ...
  try {
    // Build conversation context - convert history to OpenAI format
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        role: "system",
        content: `You are MediAI, a compassionate and highly skilled medical AI assistant. 
